...
const showEvents = (event, id) => {
  const {name, TeachersName, attendee, status, description, booked} = event
  ...
  const output = `
        <div class="card">
          <div class="card--details">
            <div>
              <h1>${name}</h1>
              <span>${attendee - booked} Number of Attendees</span>
            </div>
            <span class="card--details-ribbon ribbon-${eventStatus}">
                ${eventStatus}
            </span>
             <p>${description}</p>
            <button onclick="bookEvent(${booked} ,'${id}')" class="btn btn-tertiary">Book Appointment</button>
          </div>
        </div>
        `
